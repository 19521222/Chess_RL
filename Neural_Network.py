...
class NEURAL_NETWORK():
	...
	# calculate by policy network
        ...
        if torch.isnan(policy_loss).any():
            print('Policy Loss Nan Detected', torch.isnan(policy_loss).any(), torch.isnan(torch.log(act_probs)).any(), torch.isnan(torch.sum(probability_opt*(torch.log(act_probs)), 1)).any())
